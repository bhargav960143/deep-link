<x-layouts.app title="Edit Link" :tenant="$tenant">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="app_id" x-model="appId" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('app_id') border-red-400 @enderror">
                                @foreach($apps as $app)
                                    <option value="{{ $app->id }}" data-name="{{ $app->name }}" {{ old('app_id', $link->app_id) == $app->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="domain_id" x-model="domainId" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('domain_id') border-red-400 @enderror">
                                @foreach($domains as $domain)
                                    <option value="{{ $domain->id }}" data-domain="{{ $domain->domain }}" {{ old('domain_id', $link->domain_id) == $domain->id ? 'selected' : '' }}>
                                        {{ $domain->domain }}
                                    </option>
                                @endforeach
                            </select>
    ...
    function linkForm() {
        return {
            title: '{!! addslashes(old('title', $link->title)) !!}',
            ogTitle: '{!! addslashes(old('og_title', $link->og_title)) !!}',
            ogDescription: '{!! addslashes(old('og_description', $link->og_description)) !!}',
            ogImage: '{!! addslashes(old('og_image_url', $link->og_image_url)) !!}',
            showInterstitial: {{ old('show_interstitial', $link->show_interstitial) ? 'true' : 'false' }},
            appId: '{{ old('app_id', $link->app_id) }}',
            domainId: '{{ old('domain_id', $link->domain_id) }}',
